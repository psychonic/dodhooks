...
ISourcePawnEngine *CDetourManager::spg_pEngine = NULL;
IGameConfig *CDetourManager::gameconf = NULL;

void CDetourManager::Init(ISourcePawnEngine *spg_pEngine, IGameConfig *gameconf)
{
	CDetourManager::spg_pEngine = spg_pEngine;
...
CDetour *CDetourManager::CreateDetour(void *callbackfunction, void **trampoline, const char *signame)
{
	...
	if (detour)
	{
		if (!detour->Init(spg_pEngine, gameconf))
		{
			delete detour;
			return NULL;
		}
...
bool CDetour::Init(ISourcePawnEngine *spg_pEngine, IGameConfig *gameconf)
{
	this->spg_pEngine = spg_pEngine;
...
jit_rewind:
	...
	if (wr.outbase == NULL)
	{
		...
		wr.outbase = (jitcode_t)spg_pEngine->AllocatePageMemory(CodeSize);
		spg_pEngine->SetReadWrite(wr.outbase);
		wr.outptr = wr.outbase;
		detour_trampoline = wr.outbase;
		goto jit_rewind;
	}

	spg_pEngine->SetReadExecute(wr.outbase);
...
void CDetour::DeleteDetour()
{
	...
	if (detour_trampoline)
	{
		...
		spg_pEngine->FreePageMemory(detour_trampoline);
		detour_trampoline = NULL;
	}
}
